...
#
# Usage:
#   _scripts/lint.sh              # lint Sources/ + Tests/ (warnings + errors), concise summary
#   _scripts/lint.sh --strict     # treat every finding as an error (exit 1 on any warning)
#   _scripts/lint.sh --fix        # auto-correct the correctable violations in place
#   _scripts/lint.sh --fix --allow-dirty   # auto-correct even with uncommitted target changes
#   _scripts/lint.sh PATH ...     # restrict to the given files/dirs
#
...
# (indentation, brace placement) is owned by _scripts/format.sh; this catches the lint-only concerns.
#
...
if [ "$MODE" = "fix" ]; then
  ...
  echo "lint ✅ applied auto-corrections (re-run _scripts/lint.sh to see what remains)"
  exit 0
fi
