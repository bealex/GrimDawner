...
#   2. style-respace  — a SwiftSyntax post-pass (_scripts/StyleRespace) for the rules swift-format can't
...
#
# Usage:
#   _scripts/format.sh              # rewrite Sources/ + Tests/ in place
#   _scripts/format.sh --check      # report files that are NOT formatted; modify nothing (exit 1 if any)
#   _scripts/format.sh --allow-dirty  # rewrite even when target files have uncommitted changes
#   _scripts/format.sh PATH ...     # restrict to the given files/dirs (instead of the whole Sources/+Tests/ tree)
#
...
RESPACE_PKG="$REPO/_scripts/StyleRespace"
...
if ! TMPDIR="$RESPACE_TMP" xcrun swift build -c release --package-path "$RESPACE_PKG" --disable-sandbox >/dev/null 2>&1; then
  echo "format.sh: failed to build style-respace (_scripts/StyleRespace) — run its build manually to see why"
  exit 2
fi
...
if [ "$CHECK" = 1 ]; then
  ...
  echo "format ❌ $bad of ${#FILES[@]} file(s) need formatting — run _scripts/format.sh"
  exit 1
fi
