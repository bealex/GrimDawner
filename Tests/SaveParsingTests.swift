...
struct SaveParsingTests {
    private static var playerFile: URL? {
        // Tests/… → repository root → Resources/save
        let root = URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        let saves = root.appending(path: "Resources/save")
        guard FileManager.default.fileExists(atPath: saves.path(percentEncoded: false)) else { return nil }

        return SaveFolder.characters(in: saves).first?.playerFile
    }

    @Test
    func parsesEveryByteOfARealSave() throws {
        guard let url = Self.playerFile else { return }
