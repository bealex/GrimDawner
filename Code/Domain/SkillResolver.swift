...
struct SkillResolver {
    ...
    func looseSkills(from save: Gdc.SaveFile, claimed: Set<String>) -> [ResolvedSkill] {
        save.skills.skills
            .filter { entry in
    ...
    func changes(of modifier: ArzRecord, atLevel level: Int) -> SkillChanges {
    ...
    func masteryName(ofSkillAt path: String) -> String? {
        guard
    ...
    func skill(at path: String, level: Int) -> ResolvedSkill? {
    ...
    func parameters(of record: ArzRecord, atLevel level: Int) -> [SkillParameter] {
    ...
    func stats(of record: ArzRecord, atLevel level: Int) -> StatBlock {
        var block = StatBlock()
