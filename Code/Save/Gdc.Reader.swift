...
extension Gdc {
    ...
    struct Reader {
        ...
        mutating func flag() throws -> Bool { try byte() != 0 }
        ...
        mutating func float() throws -> Float { Float(bitPattern: try integer()) }

        /// Reads four bytes without folding them into the key — the encoding used for block lengths.
        mutating func peekInt() throws -> UInt32 {
            let slice = try take(4)
        ...
        mutating func uniqueId() throws -> [UInt8] {
            var value = [UInt8]()
        ...
        mutating func count(limit: Int = 1 << 20) throws -> Int {
            let raw = try integer()
            guard
                raw <= UInt32(limit),
                Int(raw) <= remaining + 4
        ...
        mutating func array<Element>(_ element: (inout Reader) throws -> Element) throws -> [Element] {
        ...
        mutating func blockStart(expecting expected: UInt32? = nil) throws -> Block {
            let id = try integer()
        ...
        mutating func blockEnd(_ block: Block) throws {
            guard
        ...
        mutating func skipToEnd(of block: Block) throws {
            guard
                offset <= block.endOffset
            else {
                throw Failure.blockLengthMismatch(expected: block.endOffset, actual: offset)
        ...
        mutating func trailingBytes(of block: Block) throws -> [UInt8] {
            var values = [UInt8]()
