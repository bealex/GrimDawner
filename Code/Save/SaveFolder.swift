...
struct CharacterFile: Identifiable, Hashable, Sendable {
    /// The save folder name without the underscore the game prefixes to every character directory.
    let folderName: String
    let directory: URL
    let playerFile: URL

    var id: URL { playerFile }
    var displayName: String { folderName.hasPrefix("_") ? String(folderName.dropFirst()) : folderName }
}
...
enum SaveFolder {
    static func characters(in folder: URL) -> [CharacterFile] {
