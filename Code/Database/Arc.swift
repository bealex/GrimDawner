...
struct ArcArchive: Sendable {
    ...
    var entryNames: some Collection<String> { entries.keys }
    ...
    func contains(_ name: String) -> Bool { entries[name.lowercased()] != nil }

    /// Every file the archive holds, by the name it stores them under.
    var names: some Collection<String> { entries.keys }
    ...
    func textTags() throws -> [String: String] {
