...
struct ByteView: Sendable {
    ...
    func bytes(at offset: Int, count length: Int) throws -> [UInt8] {
        try check(offset, length)
        return [UInt8](data[offset ..< offset + length])
    }

    func slice(at offset: Int, count length: Int) throws -> Data {
        try check(offset, length)
    ...
    func text(at offset: Int, count length: Int) throws -> String {
        // swiftlint:disable:next optional_data_string_conversion
        String(decoding: try slice(at: offset, count: length), as: UTF8.self)
    }

    func matches(_ ascii: [UInt8], at offset: Int) -> Bool {
