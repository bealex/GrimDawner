...
struct ArzRecord: Sendable {
    let path: String
    let recordClass: String
    let fields: [String: ArzValue]
    /// The order the record stores its fields in, which is the order the game walks them.
    let fieldOrder: [String]

    subscript(key: String) -> ArzValue? { fields[key] }

    func number(_ key: String) -> Double { fields[key]?.number ?? 0 }

    func text(_ key: String) -> String { fields[key]?.text ?? "" }

    func integer(_ key: String) -> Int { Int(fields[key]?.number ?? 0) }
}

/// A `.dbr` field value. Multi-valued fields keep every element; scalars are the single-element case.
enum ArzValue: Sendable {
    case integer([Int32])
    case real([Float])
    case text([String])
    case flag([Bool])
...
struct ArzDatabase: Sendable {
    ...
    var recordPaths: some Collection<String> { entries.keys }
    var recordCount: Int { entries.count }

    init(contentsOf url: URL) throws {
        try self.init(ByteView(contentsOf: url))
    }

    init(_ bytes: ByteView) throws {
        self.bytes = bytes
    ...
    func record(at path: String) throws -> ArzRecord? {
